...
public class UserFormUnitTest {
  ...
  @BeforeEach
  public void setUp() {
    // @BeforeEachが付与されたメソッドは、各テストメソッドを実行する前に毎回実行されるようになる
    // UserFormFactoryを用いてテストで扱うUserFormオブジェクトの生成
    userForm = UserFormFactory.createUser();

    // バリデーションチェックを行うためのオブジェクトを用意
  ...
  class ユーザーを作成できる場合 {
    ...
    public void nameとemailとpasswordとpasswordconfirmationが存在すれば登録できる () {
      ...
      // エラーが発生していないことを確認（エラーが発生している場合戻り値violationsに格納される）
